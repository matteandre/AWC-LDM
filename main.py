        ...
        for k in images:
            ...
            if isdepth(imgs):
                for i, img in enumerate(imgs[:2]):
                    img = (img + 1.) / 2.
                    elevation = parse_calib(os.path.join(Path(__file__).parent, 'lidm', 'utils', 'calib_128.json'))
                    xyz = range2pcd_channel(img, elevation, **self.dataset_kwargs)
...
if __name__ == "__main__":
    ...
    with open(opt.base[0], 'rb') as stream:
        data_loaded = stream.read()
        with open(os.path.join(logdir,'config.yaml'), 'wb') as outfile:
            outfile.write(data_loaded)
    ...
    try:
        ...
        # default to ddp
        # trainer_config["accelerator"] = "ddp" # commented
