...
class FiLMLayerWithTime(nn.Module):

    def __init__(self, _feature_size, time_embed_size):
        super(FiLMLayerWithTime, self).__init__()
        self.film_gen = linear(time_embed_size + 1, _feature_size * 2) # +1 for binary label
    
    def forward(self, feature_maps, label, time_embedding):
        label_time_concat = th.cat([label, time_embedding], dim=-1)
        film_params = self.film_gen(label_time_concat)
        gamma, beta = film_params.chunk(2, dim=-1)

        gamma = gamma.view(-1, feature_maps.size(1), 1, 1)
        beta = beta.view(-1, feature_maps.size(1), 1, 1)

        modulated_feature_maps = gamma * feature_maps + beta
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.film1 = FiLMLayerWithTime(ch, time_embed_dim)
        ...
        self.film2 = FiLMLayerWithTime(ch, time_embed_dim)
        ...
        self.film3 = FiLMLayerWithTime(model_channels, time_embed_dim)
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        :param context: conditioning plugged in via FiLM layers
        ...
        if context is not None:
            h = self.film1(h, context, emb)

        h = self.middle_block(h, emb)

        h = h.type(x.dtype)
        if context is not None:
            h = self.film2(h, context, emb)
