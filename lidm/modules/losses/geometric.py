...
from lidm.utils.boreas_utils import parse_calib
import os
...
class GeoConverter(nn.Module):
    ...
    def register_conversion(self):
        ...
        elevation = parse_calib(os.path.join(Path(__file__).parent.parent.parent, 'utils', 'calib_128.json'))
        scan_y = scan_y.flatten()
        elevation = np.array(elevation).flatten() / 180.0 * np.pi
        pitch = elevation[scan_y].reshape(yaw.shape)
