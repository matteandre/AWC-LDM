...
from ...models.autoencoder import LQModelInterface, IdentityFirstStage
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        c = batch[self.cond_stage_key][:,None].to(torch.float32)
        xc = c
        out = [z, c]
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                if isinstance(self.first_stage_model, LQModelInterface):
    ...
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False,
        ...
        b, *_, device = *x.shape, x.device
        outputs_cond = self.p_mean_variance(x=x, c=c, t=t, clip_denoised=clip_denoised,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
        outputs_uncond = self.p_mean_variance(x=x, c=None, t=t, clip_denoised=clip_denoised,
                                       quantize_denoised=quantize_denoised,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
        
        if return_x0:
            model_mean_cond, _, model_log_variance_cond, x0 = outputs_cond
        else:
            model_mean_cond, _, model_log_variance_cond = outputs_cond
        
        model_mean_uncond, _, model_log_variance_uncond = outputs_uncond

        model_mean = (self.uncond_w+1)*model_mean_cond - self.uncond_w*model_mean_uncond
        model_log_variance = (self.uncond_w+1)*model_log_variance_cond - self.uncond_w*model_log_variance_uncond
